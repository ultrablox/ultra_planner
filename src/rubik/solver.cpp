...
int main(int argc, const char ** argv)
{
	...
	{
		cout << "Rubik Solver failed to start.\n";
		cout << e.what();
		return 1;
	}
	...
	state_space_solver<problem_t> solver(in_file, cout);

	bool r = solver.solve<manhattan_3D_distance_heuristic>(storage_type.getValue() == "ext", algorithm_name.getValue());
	if (!r)
